...
// cuda_class_aglom_amg.cu: определяет точку входа для консольного приложения.
...
// например у amg1r5 быстродействие оценивается в 8с для данной задачи. Для трёхмерного Пуассона с быстродействием в
// версии 0.07 лучше чем в версии 0.06 но очень плохо по сравнению с комерческими аналогами.
// В версии 0.07 сделан прорыв в разреженном матричном умножении,алгоритм Фреда Густавсона существенно ускорен.
// Тестиование алгоритма на реальных задачах практически не было произведено по причине отсутствия реализации 
// Катхилла-Маки в реальном тестируемом приложении (AliceFlow) а данный метод my_amg_v0.07  очень чувствителен 
// к ширине ленты матрицы.
...
integer classic_aglomerative_amg1(Ak*& A, integer nnz,
	...
	*/
	...
	while (dres > tolerance) {
		...
		if (iter_limit > 1000) break; // досрочный выход из while цикла.
		iter_limit++;
...
void seidel_pressure_first(double eps,
	int imaxiter,
...
} // seidel_pressure_first


EXPORT void seidel_pressure(double eps, 
							int imaxiter,
							int imaxnumbernode_loc,
							int icolx, int icoly,
							bool bipifix_loc, 
...
{
	if (0) {
		seidel_pressure_first(eps,
			imaxiter, imaxnumbernode_loc,
			icolx, icoly, bipifix_loc, ipifix_loc,
			ibasenorma_loc, mp_loc, mapPT_loc,
			P_loc, db_loc, myrP_loc, URF);
	}
	else {
		// 13.10.2019
	...
	// с гипотезой локальности (band_size ON) : 
		...
		integer* ilink = new integer[(icolx+3)*(icoly+3)];
		for (integer i_8 = 0; i_8 < (icolx + 3) * (icoly + 3); i_8++) {
			ilink[i_8] = -1;// STUB
		}
		...
		for (int k1 = 1; k1 <= imaxnumbernode_loc; k1++) {
			if (mapPT_loc[k1].itype == 1) {
				...
				if (fabs(mp_loc[ipi].dap) > 1.0e-30) {
					...
					n_++;
					ilink[ipi] = n_;//ilink нумерация начинается с единицы.
					if (fabs(mp_loc[ipi].dae) > 1.0e-30) nnz_++;
					if (fabs(mp_loc[ipi].daw) > 1.0e-30) nnz_++;
					if (fabs(mp_loc[ipi].dan) > 1.0e-30) nnz_++;
					if (fabs(mp_loc[ipi].das) > 1.0e-30) nnz_++;
					if (fabs(mp_loc[ipi].dap) > 1.0e-30) nnz_++;						
		...
		if (1) {
			// dll code
			x = new Real[n_ + 1]; // решение.
			for (integer i = 0; i <= n_; i++) x[i] = 0.0; // инициалзация.
			b = new Real[n_ + 1]; // Правая часть.
			b[0] = 0.0;
			...
			for (int k1 = 1; k1 <= imaxnumbernode_loc; k1++) {
				if (mapPT_loc[k1].itype == 1) {
					...
					if (fabs(mp_loc[ipi].dap) > 1.0e-30) {
						...
						b[ilink[ipi]] = db_loc[ipi];
						//for (integer i_1 = 1; i_1 <= n_; i_1++) {
							//b[i_1] = db[i_1 - 1];
						//}

						A[i_2].aij = mp_loc[ipi].dap;// MatrixAmg[i_1 - 1].val;
						A[i_2].i = ilink[ipi]; //MatrixAmg[i_1 - 1].i;
						A[i_2].j = ilink[ipi]; //MatrixAmg[i_1 - 1].j;
						i_2++;

						if (fabs(mp_loc[ipi].dae) > 1.0e-30) {
							A[i_2].aij = -mp_loc[ipi].dae;// MatrixAmg[i_1 - 1].val;
							A[i_2].i = ilink[ipi]; //MatrixAmg[i_1 - 1].i;
							A[i_2].j = ilink[iei]; //MatrixAmg[i_1 - 1].j;
							i_2++;
						}

						if (fabs(mp_loc[ipi].daw) > 1.0e-30) {
							A[i_2].aij = -mp_loc[ipi].daw;// MatrixAmg[i_1 - 1].val;
							A[i_2].i = ilink[ipi]; //MatrixAmg[i_1 - 1].i;
							A[i_2].j = ilink[iwi]; //MatrixAmg[i_1 - 1].j;
							i_2++;
						}

						if (fabs(mp_loc[ipi].dan) > 1.0e-30) {
							A[i_2].aij = -mp_loc[ipi].dan;// MatrixAmg[i_1 - 1].val;
							A[i_2].i = ilink[ipi]; //MatrixAmg[i_1 - 1].i;
							A[i_2].j = ilink[ini]; //MatrixAmg[i_1 - 1].j;
							i_2++;
						}

						if (fabs(mp_loc[ipi].das) > 1.0e-30) {
							A[i_2].aij = -mp_loc[ipi].das;// MatrixAmg[i_1 - 1].val;
							A[i_2].i = ilink[ipi]; //MatrixAmg[i_1 - 1].i;
							A[i_2].j = ilink[isi]; //MatrixAmg[i_1 - 1].j;
							i_2++;
						}

						/*
							P_loc[ipi] = P_loc[ipi] +
								URF * ((mp_loc[ipi].dae * P_loc[iei]
									+ mp_loc[ipi].daw * P_loc[iwi]
									+ mp_loc[ipi].dan * P_loc[ini]
									+ mp_loc[ipi].das * P_loc[isi]
									+ db_loc[ipi]) /
									mp_loc[ipi].dap -
									P_loc[ipi]);
						*/
			...
			bool bamgdivergencedetected;
			classic_aglomerative_amg1(A, nnz_, n_, R, P,
				Atemp, Atemp2, x, b, theta, bamgdivergencedetected);
			
			
			if (bamgdivergencedetected) {
				seidel_pressure_first(eps,
					imaxiter, imaxnumbernode_loc,
					icolx, icoly, bipifix_loc, ipifix_loc,
					ibasenorma_loc, mp_loc, mapPT_loc,
				...
				for (int k1 = 1; k1 <= imaxnumbernode_loc; k1++) {
					if (mapPT_loc[k1].itype == 1) {
						...
						if (fabs(mp_loc[ipi].dap) > 1.0e-30) {
							...
							// В х нумерация начинается с единицы.
							P_loc[ipi] = x[ilink[ipi]];
						}
					}
				}
			}
...
EXPORT void seidel_uds(double eps, 
							int imaxiter,
							int imaxnumbernode_loc,
							int icolx, int icoly,
							int ibasenorma_loc,
							...
							int icurentuds)
{
	...
	// граничном условии продолжения уравнения вплоть до самой границы.
